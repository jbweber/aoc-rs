...
fn process_muls(input: &str) -> Vec<(u32, u32)> {
    ...
    for captures in re.captures_iter(input) {
        if captures.get(0).map(|m| m.as_str()) == Some("do()") {
            on = true;
            continue;
        }

        if captures.get(0).map(|m| m.as_str()) == Some("don't()") {
            on = false;
            continue;
        }
...
mod tests {
    ...
    #[case("xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))", vec![(2,4),(8,5)])]
