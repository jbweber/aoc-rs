...
pub fn process(input: &str) -> anyhow::Result<String> {
    ...
    for print in bad_prints.iter_mut() {
        print.sort_by(|a, b| {
            if is_before(*a, *b, &mapping) {
                std::cmp::Ordering::Greater
            } else if is_before(*b, *a, &mapping) {
    ...
    let result = bad_prints.iter().map(|p| p[p.len() / 2]).sum::<u32>();

    Ok(result.to_string())
}

fn is_before(a: u32, b: u32, rules: &HashMap<u32, Vec<u32>>) -> bool {
